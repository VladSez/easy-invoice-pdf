...
// kept out of the test body so the language branch is not a conditional inside
// `it`, and so the Polish data is still only built when the test actually runs
function getExportInvoiceData() {
  return exportLanguage === "pl"
...
describe.skipIf(!exportPath)("export server PDF for e2e", () => {
  it("writes PDF buffer to disk", async () => {
    const invoiceData = getExportInvoiceData();
